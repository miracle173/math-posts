...
class SegmentedMatrix:
    ...
    # 2025-12-20 16:48 begin
    @property
    def Id(self) -> str:
        """
        String-ID der SegmentedMatrix:
        - zuerst alle Matrixelemente zeilenweise, ohne Trennzeichen
        - dann ':' + alle bandWidths
        - dann ':' + alle stackWidths
        """

        # 1) Matrixinhalt (Zeile für Zeile)
        cells = ''.join(
            str(self.data[r][c])
        ...
        # 2) BandWidths
        bands = ''.join(str(bw) for bw in self.bandWidths)

        # 3) StackWidths
        stacks = ''.join(str(sw) for sw in self.stackWidths)

        return f"{cells}:{bands}:{stacks}"
    # 2025-12-20 16:48 end

    # 2025-12-20 17:02 begin
    @classmethod
    def fromId(cls, myId: str):
        """
        Erzeugt eine SegmentedMatrix aus einer Id,
        wie sie von der Property `Id` erzeugt wird.
        """

        try:
            cell_part, band_part, stack_part = myId.split(":")
        except ValueError:
            raise ValueError("Ungültiges Id-Format (erwarte cells:bands:stacks)")

        # --- bandWidths und stackWidths ---
        bandWidths = [int(ch) for ch in band_part]
        stackWidths = [int(ch) for ch in stack_part]

        # --- Dimensionen ---
        nrows = sum(bandWidths)
        ncols = sum(stackWidths)

        if len(cell_part) != nrows * ncols:
            raise ValueError(
                f"Zellteil hat Länge {len(cell_part)}, "
                f"erwartet {nrows*ncols}"
            )

        # --- Matrix erzeugen ---
        sm = cls(bandWidths, stackWidths)

        # --- Zellen rekonstruieren ---
        it = iter(cell_part)
        sm.data = [
            [int(next(it)) for _ in range(ncols)]
            for _ in range(nrows)
        ]

        return sm
    # 2025-12-20 17:02 end
    ...
    def generate_neighbors(self, permuteSymbols=False):
        ...
        Erzeugendensystem:
          ...
          - Band 0 <-> k
          - Stack 0 <-> k
          - Row 0 <-> k innerhalb eines Bands
          - Col 0 <-> k innerhalb eines Stacks
          - optionale Symbolpermutation (nur Wertetausch)
        """
        ...
        if permuteSymbols:
            ...
            if len(symbols) >= 2:
                ...
                for other in symbols[1:]:
                    ...
                    m.data = [
                        [
                            base if v == other else
                            other if v == base else
                            v
                            for v in row
                        ]
                        for row in self.data
...
##        maxima = cls.filterMaximalSegmentedMatrices(no_isolated_singletons)
        maxima = cls.filterMaximalSegmentedMatrices(all_binary)  # !!! ersetzt obigen Block mit isolated singletons
    ...
    # 2025-12-20 16:35 begin
    def _replaceBySymbols(self, aSymbolList):
        """
        Ersetzt die Einsen dieser Matrix durch die Werte aus aSymbolList
        in Block→Zeilen→Spalten-Reihenfolge.
        Gibt die neue Matrix zurück oder None, falls inkonsistent.
        """
        ...
        for bw in self.bandWidths:
            ...
            for sw in self.stackWidths:
                for i in range(bw):
                    for j in range(sw):
                        if self.data[row_start + i][col_start + j] == 1:
                            one_positions.append((row_start + i, col_start + j))
                col_start += sw
            row_start += bw

        if len(one_positions) != len(aSymbolList):
            return None

        for (r, c), value in zip(one_positions, aSymbolList):
            sm.data[r][c] = value
        ...
        return sm
    # 2025-12-20 16:35 end

    # 2025-12-20 16:35 begin
    def _applySymbolLists(self, aListOfSymbolLists):
        """
        Wendet alle Symbolfolgen aus aListOfSymbolLists auf diese
        0-1-SegmentedMatrix an und liefert alle konsistenten Ergebnisse.
        """

        results = []

        for symbol_list in aListOfSymbolLists:
            sm = self._replaceBySymbols(symbol_list)
            if sm is not None:
                results.append(sm)
    ...
    def allSymbolRepresentatives(cls, nClues: int):
        ...
        # 1) Restricted Growth Sequences
        rgs_list = cls.restrictedGrowthSequence(nClues)
        rgs_list = [[nClues + 1 - s for s in rgs] for rgs in rgs_list]

        # 2) 0-1-Zellrepräsentanten
        ...
        # 3) Symbolersetzung anwenden
        for base in base_matrices:
            collected.extend(base._applySymbolLists(rgs_list))
    ...
    def matchesClueMatrixOrRaise(self, clueMatrix):
        ...
        while queue:
            myTrials+=1
            if myTrials>self.MaxTrials:
                raise ValueError("Matrix nach "+str(self.MaxTrials)+" Versuchen aufgegeben.")
    ...
    # 2025-12-20 17:41 begin
    @property
    def newSortId(self):
        """
        Sortierschlüssel gemäß spezifizierter 6-stufiger Ordnung.
        """

        # ---------------------------------------------------------
        # 0) Matrix auf 3×3 Blöcke / 9×9 Zellen expandieren
        # ---------------------------------------------------------
        ...
        for b, bw in enumerate(self.bandWidths):
            ...
            for s, sw in enumerate(self.stackWidths):
                for i in range(bw):
                    for j in range(sw):
                        expanded[row0+i][col0+j] = self.data[row0+i][col0+j]
                col0 += sw
            row0 += bw
        ...
        for r in range(9):
            for c in range(9):
                blocks[block_index(r, c)].append(expanded[r][c])

        # ---------------------------------------------------------
        # 1) Anzahl belegter Blöcke (absteigend)
        # ---------------------------------------------------------
        block_occupied = [any(v != 0 for v in blk) for blk in blocks]
        key1 = sum(block_occupied)

        # ---------------------------------------------------------
        # 2) Belegte Blöcke als Binärstring (expandiert!)
        # ---------------------------------------------------------
        key2_bits = ''.join('1' if x else '0' for x in block_occupied)

        # ---------------------------------------------------------
        # 3) Anzahl Clues pro Block (nichtleere Blöcke, Reihenfolge wie 2)
        # ---------------------------------------------------------
        key3 = tuple(
            sum(1 for v in blocks[i] if v != 0)
            for i in range(9) if block_occupied[i]
        )

        # ---------------------------------------------------------
        # 4) Belegte Zellen binär, Block für Block, expandiert
        # ---------------------------------------------------------
        key4 = ''.join(
            '1' if v != 0 else '0'
            for i in range(9)
        ...
        # ---------------------------------------------------------
        # 5) Werte substituieren → 1,2,3,... dann (nClues+1 - x)
        # ---------------------------------------------------------
        values_in_order = [
            v for i in range(9)
            for v in blocks[i]
        ...
        # ---------------------------------------------------------
        # 6) Originalwerte (nur nichtleere), gleiche Reihenfolge
        # ---------------------------------------------------------
        key6 = tuple(values_in_order)

        # ---------------------------------------------------------
        # Gesamt-Sortierschlüssel
        # (Minuszeichen für absteigende Sortierung)
        # ---------------------------------------------------------
        return (
            -key1,
            key2_bits,
            tuple(-x for x in key3),
            key4,
            key5,
            key6
        )
    # 2025-12-20 17:41 end
