...
But here is a different way to prove the statement. We prove the more specifific statement:
...
**Statement 2**  
If there is a sequence $(c_0, c_1, \dots, c_8)$, where $c_i\in\{0,1\}, \forall  i \in \{0,\dots,8\} $, then there exist three indices $i,m,j \in \{0,\dots,8\}$ such that  $i<j$, $m=(i+j)/2$ and $c_i=c_m=c_j$.
